...
export function CheckedOutBookList({ person, books, dataFunctions }) {
  ...
  const personBookReferences = books.filter(
    (reference) => reference.checkedOutByPersonId === person.id,
  );

  personBookReferences.forEach((book) => {
  ...
  return (
    <>
      <span className="badge bg-secondary">{personBookReferences.length}</span>{" "}
      checked out books &nbsp;
